export default function ImageChevron() {
    return (
        <>
            <Chevron isLeft={true}/>
...
function Chevron({ isLeft }) {
    return (
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" className={`absolute w-7 h-7 flex justify-center items-center top-1/2 rounded-full -translate-y-1/2 cursor-pointer ${isLeft? "rotate-180 left-0" : "right-0"}`}>
            ...
            <rect width="100%" height="100%" fill="white" mask={`url(#mask-${isLeft? "left" : "right"})`} className="transition opacity-40 hover:opacity-35"/>
        </svg>
    )
}